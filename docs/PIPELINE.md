...
- **Risk Assessment** - Classifies changes as none, low, medium, high, or critical risk
- **Safety Verdict** - Determines if deployment should proceed
...
**Recommendations:**
- Verify the Application Insights instance exists before deploying
- Confirm the sampling rate of 100% is appropriate for production
...
Configure `--risk-threshold` to control when deployments are blocked:

```bash
--risk-threshold low       # Block on low risk and above (very strict)
--risk-threshold medium    # Block on medium risk and above (strict)
--risk-threshold high      # Block on high risk and above (recommended)
--risk-threshold critical  # Block only on critical risk (permissive)
```

### Risk Level Definitions

- **Critical:** Deletion of stateful resources (databases, storage, key vaults), identity/RBAC deletions, encryption changes
- **High:** Production resource deletions, auth config changes, firewall changes, SKU downgrades
- **Medium:** Behavioral changes to existing resources, new public endpoints
- **Low:** Adding resources, tags, monitoring, cosmetic changes
- **None:** NoChange, Ignore, no impact
...
### 3. Set Appropriate Risk Threshold

- **Development/Staging:** Use `medium` or `low`
- **Production:** Use `high` or `critical`
...
```bash
az deployment group what-if ... | whatif-explain --ci --diff-ref main
```
...
- name: Review (Dev)
  ...
  run: whatif-explain --ci --risk-threshold medium

- name: Review (Prod)
  if: contains(github.ref, 'main')
  run: whatif-explain --ci --risk-threshold critical
```
...
- Check `--risk-threshold` setting
- Review the verdict in output or PR comment
- The LLM may be overly conservative—adjust threshold if needed
